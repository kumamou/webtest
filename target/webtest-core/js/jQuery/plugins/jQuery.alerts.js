(function($) {
	...
	$.alerts = {
        ...
        warn: function(message, title, callback) {
            if( title == null ) title = 'Warn';
            $.alerts._show(title, message, null, 'warn', function(result) {
                if( callback ) callback(result);
            });
        },
...
//					$("#popup_title").css({ cursor: 'move' });
                    //绑定拖动
                    var popup_box =  $("#popup_container");
                    if(popup_box.length != 0){
                        popup_box.find("#popup_title").myDrag(popup_box);
                    }
